...
const PodcastPlayer = () => {
  ...
  const [podcastTitleLink, setPodcastTitleLink] = useState<string | undefined>(
    undefined
  );
  ...
  const loadPodcastUrl = async () => {
    try {
      ...
      const podcastPosts = parsedFeed.items
        .filter((item) =>
      ...
      if (mostRecentPodcast != null) {
        const podcastUrl = mostRecentPodcast.enclosures.find(
          (enclosure) => enclosure.mimeType === "audio/mpeg"
        )?.url;
        if (podcastUrl != null) {
          ...
          setPodcastTitleLink(
            mostRecentPodcast.links.find(
              (link) =>
                (link as { url: unknown }).url !== null && link.url !== ""
            )?.url
          );
        }
      }
