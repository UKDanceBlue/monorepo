...
export const FirestoreImageView = (props: // Exclude<
...
// > &
{
  ...
  const [downloadableImage, setDownloadableImage] =
    useState<DownloadableImage>();

  useEffect(() => {
    if (!isFirestoreImage(props.source)) {
      return;
    } else {
      parseFirestoreImage(props.source, fbStorage)
        .then(setDownloadableImage)
        .catch(universalCatch);
    }
  }, [fbStorage, props.source]);

  return downloadableImage == null ? (
    <Center>
      <Spinner />
    </Center>
  ) : (
