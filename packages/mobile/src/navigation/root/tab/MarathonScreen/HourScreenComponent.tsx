...
export const HourScreenComponent = () => {
  ...
  // firestore hour
  const [isLoading, errorMessage, hourData, hourImage, refresh] =
  ...
  if (hourData != null) {
    ...
    hourDataComponent = <Text variant="error-message">No hour data</Text>;
  }
  ...
  return (
    <VStack flex={1} alignItems="center" flexDirection="column">
      {hourImageComponent}
      {errorMessage != null ? (
        <Text flex={1} variant="error-message">
          {errorMessage}
        </Text>
      ) : null}
