...
const TeamScreen = () => {
  const { team, linkblue: userLinkblue } = useUserData();
  ...
  if (team == null) {
    ...
    return (
      <TeamInformation
        // TODO ADD CAPTAINS
        captains={captains
          .map((linkblue) => memberNames[linkblue])
          .filter((name): name is string => name != null)}
        members={Object.values(memberNames).filter(
        ...
        scoreboardData={
          individualTotals == null
            ? []
            : Object.entries(individualTotals)
                .filter(([linkblue]) => linkblue !== "%TEAM%")
                .map(([linkblue, points]) => {
                  return {
                    id: linkblue,
