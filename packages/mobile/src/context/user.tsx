...
const loadUserData = async (
  ...
  switch (loginType) {
    ...
    case "ms-oath-linkblue": {
      ...
      loadedUserData.notificationReferences =
        rawUserData.notificationReferences ?? [];
...
export const UserDataProvider = ({
  ...
  const { isLoggedIn, uid, isAnonymous, isAuthLoaded } = useAuthData();
  ...
  const refresh = useCallback(() => {
    if (isAuthLoaded && isLoggedIn && uid != null) {
      ...
      return loadUserData(
        uid,
        isAnonymous ? "anonymous" : "ms-oath-linkblue",
        fbFirestore
      )
        .then(setUserData)
