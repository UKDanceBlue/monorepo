...
const demoModeState: AppConfiguration = {
  ...initialState,
  isConfigLoaded: true,
  enabledScreens: ["Events", "Scoreboard", "Team", "MarathonHours"],
  scoreboardMode: initialState.scoreboardMode,
  allowedLoginTypes: [],
};

const AppConfigContext = createContext<
  [AppConfiguration, (key: string) => boolean]
>([initialState, () => false]);
...
const updateState = async (
  ...
  try {
    await fbRemoteConfig.setDefaults({
      shown_tabs: "[]",
      rn_scoreboard_mode:
        '{"pointType":"spirit","showIcons":false,"showTrophies":true}',
    ...
    try {
      ...
      if (!activated) {
        log(
          `Remote config not activated, last fetch status was ${fbRemoteConfig.lastFetchStatus}`
        );
      } else {
    ...
    try {
      parsedRemoteConfig.enabledScreens = (JSON.parse(
        remoteConfigData.shown_tabs.asString()
    ...
    try {
      parsedRemoteConfig.fancyTab = remoteConfigData.fancy_tab.asString() as
        | string
        | undefined;
    } catch (error) {
      if (error instanceof SyntaxError) {
    ...
    try {
      parsedRemoteConfig.allowedLoginTypes =
        (JSON.parse(remoteConfigData.login_mode.asString()) as
          | UserLoginType[]
          | undefined) ?? undefined;
    } catch (error) {
      if (error instanceof SyntaxError) {
    ...
    parsedRemoteConfig.demoModeKey =
      remoteConfigData.demo_mode_key.asString() as string | undefined;
    try {
      parsedRemoteConfig.scoreboardMode = (JSON.parse(
        remoteConfigData.rn_scoreboard_mode.asString()
      ) ?? undefined) as
        | {
            pointType: string;
            showIcons: boolean;
    ...
    return {
      isConfigLoaded: true,
      allowedLoginTypes:
        parsedRemoteConfig.allowedLoginTypes ?? initialState.allowedLoginTypes,
      enabledScreens:
        parsedRemoteConfig.enabledScreens ?? initialState.enabledScreens,
      ...
      scoreboardMode:
        parsedRemoteConfig.scoreboardMode ?? initialState.scoreboardMode,
    };
  } finally {
    setLoading(false);
  }
...
export const AppConfigProvider = ({
  ...
  useEffect(() => {
    updateState(setLoading, fbRemoteConfig)
      .then(setConfigData)
      .catch(universalCatch);
  }, [fbRemoteConfig, setLoading]);

  const tryToSetDemoMode = useCallback(
    (key: string, demoModeKey: string): boolean => {
  ...
  return (
    <AppConfigContext.Provider
      value={[configData, (key: string) => tryToSetDemoMode(key, demoModeKey)]}
    >
      {children}
    </AppConfigContext.Provider>
  );
